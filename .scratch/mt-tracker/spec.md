...
- **Bare `mt`:** sem comando (após extrair `@bookmark`), `mt` executa a listagem `in_progress` do vault resolvido — estritamente a saída de `mt list --status in_progress`: mesmas linhas, mesmos sufixos `[blocked]`/`[defer ...]`, mesmo warning de Rank duplicado no stderr (calculado sobre o vault inteiro). Gatilho = nenhum comando restante após extrair `@bookmark` (cobre `mt`, `mt @bjd` e `mt --vault <path>`). Nenhuma issue `in_progress` → stdout vazio, exit 0, sem dica. Sem vault resolvível → exit 1 com as instruções de resolução (sem fallback para a ajuda). Flags do `list` não sobem pro root (`mt --status open` é erro de uso, exit 2). Status personalizados nunca aparecem no bare `mt` — a equivalência é com o status literal `in_progress`. `mt help`/`mt --help` continuam sendo o caminho para a ajuda raiz. Sem ADR (default reversível): README + `rootLong` como registro.
